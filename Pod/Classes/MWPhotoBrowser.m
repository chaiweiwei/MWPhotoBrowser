...
- (void)setNavBarAppearance:(BOOL)animated {
    ...
    navBar.barTintColor = [UIColor colorWithRed:229/255.0 green:68/255.0 blue:64/255.0 alpha:1];
...
- (void)tilePages {
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            bool showSelectedButton = [self showSelectedButtonStatus];
            // Add selected button
...
- (void)showGridAnimated {
    //不知道为什么偏移量有问题
    _currentGridContentOffset.y += 44 + [UIApplication sharedApplication].statusBarFrame.size.height;
    [self showGrid:YES];
}
...
- (BOOL)showSelectedButtonStatus {
    bool showSelectedButton = self.displaySelectionButtons;
    if ([self.delegate respondsToSelector:@selector(photoBrowser:canSelectedAtIndex:)]) {
        showSelectedButton = [self.delegate photoBrowser:self canSelectedAtIndex:_currentPageIndex];
    }
